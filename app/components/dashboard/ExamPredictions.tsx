    ...
    return (
        <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-5 flex flex-col h-full hover:bg-slate-800/60 hover:-translate-y-1 hover:shadow-lg hover:shadow-indigo-500/10 transition-all duration-200 group">
            ...
            <div className="flex flex-col overflow-hidden rounded-2xl border border-slate-700/50">
                ...
                <div className="flex flex-col bg-slate-800/20 divide-y divide-slate-700/50 min-h-[160px]">
                    {isLoading ? (
                        <div className="flex-1 flex items-center justify-center text-sm font-medium text-slate-500 animate-pulse py-8">
                            Analyzing document frequencies...
                        </div>
                    ) : predictions.length === 0 ? (
                        <div className="flex-1 flex flex-col items-center justify-center py-6 text-center">
                            <h3 className="text-slate-400 font-bold mb-1">No Data Available</h3>
                            <p className="text-xs text-slate-500 px-4">Upload previous year papers or course syllabus files to generate topic predictions.</p>
                        </div>
                    ) : (
                        predictions.map((item) => (
                            <div key={item.id} className="grid grid-cols-12 gap-4 p-4 items-center hover:bg-slate-800/40 transition-colors">
                                <div className="col-span-6 font-semibold text-slate-200 text-sm truncate pl-1" title={item.topic}>
                                    {item.topic}
                                </div>
